...
int SDIO_Init(sdio_cfg_t *cfg) {
    ...
    if ((rv = SDIO_ClockEnable(cfg)) != 0) {
        return rv;
    }

    if ((rv = GPIO_Init(cfg->ck_cfg)) != 0) {
        return rv;
    }
    if ((rv = GPIO_Init(cfg->cmd_cfg)) != 0) {
        return rv;
    }
    if ((rv = GPIO_Init(cfg->d0_cfg)) != 0) {
        return rv;
    }
    if ((rv = GPIO_Init(cfg->d1_cfg)) != 0) {
        return rv;
    }
    if ((rv = GPIO_Init(cfg->d2_cfg)) != 0) {
        return rv;
    }
    if ((rv = GPIO_Init(cfg->d3_cfg)) != 0) {
        return rv;
    }
    if ((rv = GPIO_Init(cfg->cd_cfg)) != 0) {
        return rv;
    }
    ...
    cfg->inst.SD_InitStruct.Init.ClockPowerSave =
        SDMMC_CLOCK_POWER_SAVE_DISABLE;
    cfg->inst.SD_InitStruct.Init.BusWide = SDMMC_BUS_WIDE_1B;
    cfg->inst.SD_InitStruct.Init.HardwareFlowControl =
    ...
    if (HAL_SD_Init(&cfg->inst.SD_InitStruct) != HAL_OK) {
        return EINVAL;
    }

    if (HAL_SD_ConfigWideBusOperation(&cfg->inst.SD_InitStruct,
