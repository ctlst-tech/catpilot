...
int DMA_Init(dma_cfg_t *cfg) {
    ...
    if ((rv = DMA_ClockEnable(cfg)) != 0) {
        return rv;
    }
    if ((rv = DMA_EnableIRQ(cfg)) != 0) {
        return rv;
    }

    rv = HAL_DMA_Init(&cfg->DMA_InitStruct);

    return rv;
}

int DMA_ReInit(dma_cfg_t *cfg) {
    if (HAL_DMA_DeInit(&cfg->DMA_InitStruct) != HAL_OK) {
        return EINVAL;
    }
    if (HAL_DMA_Init(&cfg->DMA_InitStruct) != HAL_OK) {
        return EINVAL;
    }
    return 0;
}
