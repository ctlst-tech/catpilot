...
int ADC_Init(adc_cfg_t *cfg) {
    ...
    if((rv = ADC_ClockEnable(cfg)) != 0) return rv;
    if((rv = ADC_EnableIRQ(cfg)) != 0) return rv;

    if((rv = HAL_ADC_Init(&cfg->ADC_InitStruct)) != 0) return rv;
    ...
    rv = HAL_ADC_Start_DMA(&cfg->ADC_InitStruct, (uint32_t *)cfg->buf, cfg->ch_num);
