...
// Private functions
static void ak09918_fsm(void *area);
static int ak09918_configure(ak09918_t *dev);
static uint8_t ak09918_read_reg(ak09918_t *dev, uint8_t reg);
static void ak09918_write_reg(ak09918_t *dev, uint8_t reg, uint8_t value);
static void ak09918_set_clear_reg(ak09918_t *dev, uint8_t reg, uint8_t setbits,
                                 uint8_t clearbits);
static void ak09918_read_meas(ak09918_t *dev);
static int ak09918_process_meas(ak09918_t *dev);
static void ak09918_update_meas(ak09918_t *dev);
...
ak09918_t *ak09918_start(char *name, uint32_t period, uint32_t priority,
    ...
    ak09918_t *dev = calloc(1, sizeof(ak09918_t));
    if (dev == NULL) {
        return NULL;
    }
    ...
    if ((dev->service = service_start(dev->name, dev, ak09918_fsm, period,
                                    priority)) == NULL) {
...
static void ak09918_fsm(void *area) {
    ak09918_t *dev = (ak09918_t *)area;
    switch (dev->state) {
        case AK09918_RESET:
            ak09918_write_reg(dev, CNTL2, SRST);
            vTaskDelay(100);
            dev->state = AK09918_RESET_WAIT;
            break;

        case AK09918_RESET_WAIT:
            if ((ak09918_read_reg(dev, WIA2) == DEVICE_ID) &&
                ((ak09918_read_reg(dev, CNTL2) & SRST) == 0)) {
                dev->state = AK09918_CONF;
                LOG_DEBUG(dev->name, "Device available");
            } else {
                dev->state = AK09918_RESET;
                dev->attempt++;
                if (dev->attempt > 5) {
                    dev->state = AK09918_FAIL;
        ...
        case AK09918_CONF:
            if (ak09918_configure(dev)) {
                dev->state = AK09918_MEAS;
        ...
        case AK09918_MEAS:
            ak09918_write_reg(dev, CNTL1, MODE_CONT_MEAS_4);
            dev->state = AK09918_READ;
            break;

        case AK09918_READ:
            ak09918_read_meas(dev);
            ak09918_process_meas(dev);
...
static void ak09918_set_clear_reg(ak09918_t *dev, uint8_t reg, uint8_t setbits,
                                 uint8_t clearbits) {
    uint8_t orig_val = ak09918_read_reg(dev, reg);
    uint8_t val = (orig_val & ~clearbits) | setbits;
    if (orig_val != val) {
        ak09918_write_reg(dev, reg, val);
    }
}
...
static int ak09918_configure(ak09918_t *dev) {
    ...
    for (int i = 0; i < SIZE_REG_CFG; i++) {
        ak09918_set_clear_reg(dev, reg_cfg[i].reg, reg_cfg[i].setbits,
                             reg_cfg[i].clearbits);
    }

    // Verify configuration
    for (int i = 0; i < SIZE_REG_CFG; i++) {
        orig_val = ak09918_read_reg(dev, reg_cfg[i].reg);
...
void ak09918_get_meas_non_block(ak09918_t *dev, void *ptr) {
    ...
    memcpy(ptr, (void *)&dev->meas, sizeof(ak09918_meas_t));
    xSemaphoreGive(dev->sync.mutex);
}

void ak09918_get_meas_block(ak09918_t *dev, void *ptr) {
    xSemaphoreTake(dev->sync.measrdy_sem, portMAX_DELAY);
    ak09918_get_meas_non_block(dev, ptr);
}

void ak09918_stat(ak09918_t *dev) {
    if (dev == NULL || dev->state != AK09918_READ) {
        return;
    }
